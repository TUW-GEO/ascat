...
class TestOrthoMultiCellFile(unittest.TestCase):
    ...
    def test_bla(self):
        ...
        # path_to_ascat = "/data-write/RADAR/hsaf/h121_v1.0/ts/"
        # h121 = CellGridFiles.from_product_id(path_to_ascat, "h121_v1.0")
        # h121_ds = h121.read(bbox=bbox)
        # print(h121_ds)
...
class TestCellGridFiles(unittest.TestCase):
    ...
    def test_real_select_one_lid(self):
        from time import time
        cells_path = Path("/data-write/RADAR/hsaf/h121_v2.0/time_series/")
        collection = CellGridFiles.from_product_id(cells_path, "h121_v2.0")
        lids_2588 = collection.grid.grid_points_for_cell(2588)[0]
        # first time slightly slow because we get the grid from the registry
        t1 = time()
        ds_single_pt = collection.read(location_id=lids_2588[len(lids_2588)//2])
        t2 = time()
        print(t2-t1)

        t1 = time()
        ds_single_pt2 = collection.read(location_id=lids_2588[len(lids_2588)//2+10])
        t2 = time()
        print(ds_single_pt)
        print(ds_single_pt2)
        print(xr.concat([ds_single_pt, ds_single_pt2], dim="obs"))
        print(t2-t1)

    def test_real_select_several_lids(self):
        from time import time
        print("contiguous")
        cells_path = Path("/data-write/RADAR/hsaf/h121_v2.0/time_series/")
        collection = CellGridFiles.from_product_id(cells_path, "h121_v2.0")
        lids_2588 = collection.grid.grid_points_for_cell(2588)[0]
        # t1 = time()
        # ds_several_pts = collection.read(location_id=lids_2588[0:100]).load()
        # t2 = time()
        # print(t2-t1)
        # print(ds_several_pts)

        # t1 = time()
        # [collection.read(location_id=[lid]).load() for lid in lids_2588[0:100]]
        # t2 = time()
        # print(t2-t1)
        ...
        t1 = time()
        ds = collection.read(cell=[2588], date_range=dates).load()
        t2 = time()
        print(ds)
        print(t2-t1)
        #print(ds.cf_geom.to_indexed_ragged().load())

        # print("indexed")
        # cells_path = Path("/data-write/RADAR/hsaf/h121_v1.0/ts/")
        # collection = CellGridFiles.from_product_id(cells_path, "h121_v1.0")
        # lids_2588 = collection.grid.grid_points_for_cell(2588)[0]
        # t1 = time()
        # ds_several_pts = collection.read(location_id=lids_2588[145:150])#.load()
        # t2 = time()
        # print(t2-t1)
        # print(ds_several_pts)

        # t1 = time()
        # [collection.read(location_id=[lid]) for lid in lids_2588[145:150]]
        # t2 = time()
        # print(t2-t1)


    def test_real_select_one_cell(self):
        from time import time
        cells_path = Path("/data-write/RADAR/hsaf/h121_v2.0/time_series/")
        collection = CellGridFiles.from_product_id(cells_path, "h121_v2.0")
        # path to cell
        path_2588 = collection.fn_search(cell=2588)[0]

        t1 = time()
        print(xr.open_dataset(path_2588).load())
        t2 = time()
        print(t2-t1)

        # time to load one cell with reader
        # t1 = time()
        # ds_cell = collection.read(cell=[2588]).load()
        # t2 = time()
        # print(t2-t1)

    def test_real_select_several_cells(self):
        from time import time
        cells_path = Path("/data-write/RADAR/hsaf/h121_v2.0/time_series/")
        collection = CellGridFiles.from_product_id(cells_path, "h121_v2.0")
        # path to cell
        # path_2587 = collection.fn_search(cell=2587)[0]
        # path_2588 = collection.fn_search(cell=2588)[0]

        # t1 = time()
        # print(xr.open_dataset(path_2588).load())
        # print(xr.open_dataset(path_2587).load())
        # t2 = time()
        # print(t2-t1)

        # time to load one cell with reader
        # t1 = time()
        ds_cell = collection.read(cell=[2587, 2588]).cf_geom.to_indexed_ragged().load()
        # t2 = time()
        # print(t2-t1)

    def test_convert_to_indexed_then_load(self):
        from time import time
        cells_path = Path("/data-write/RADAR/hsaf/h121_v2.0/time_series/")
        collection = CellGridFiles.from_product_id(cells_path, "h121_v2.0")
        # path to cell
    ...
    def test_real_select_bbox(self):
        from time import time
        cells_path = Path("/data-write/RADAR/hsaf/h121_v2.0/time_series/")
        collection = CellGridFiles.from_product_id(cells_path, "h121_v2.0")
        # path to cell
        ...
        # time to load one cell with reader
        t1 = time()
        ds_cell = collection.read(cell=[2587, 2588])#.load()
        t2 = time()
        print(t2-t1)
